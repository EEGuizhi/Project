...
class SaveManager(object):
    ...
    def config_name2value(self):
        ...
        if self.config.Hint.num_dist == 'datset16':  # hint 的次數的機率
    ...
    def read_config(self, arg):
        # load config
        if arg.only_test_version:  # 有傳入"只做test"的參數
        ...
        self.config = Munch.fromDict(config)  # dictionary轉munch
        ...
        # set path and write version (應該是設一些為了存檔之類的路徑,版本而創的字串)
    ...
    def save_config(self):
        with open(self.config.PATH.CONFIG_PATH, 'w') as f:
            yaml.dump(self.config.toDict(), f)  # https://shengyu7697.github.io/python-write-yaml-file/
        return
    ...
    # ===== Added =====
    def save_detect_pred(self, save_path=None):
        if save_path == None: save_path = self.config.PATH.PREDICTION_RESULT_PATH.replace(".pickle", ".json")
        elif save_path[-5:] != ".json": save_path += ".json"

        data = json.dumps(self.predictions_for_save, indent=4)
        with open(save_path, "w") as outfile:
            outfile.write(data)

    def add_detect_pred_for_save():
        pass
    # =================
