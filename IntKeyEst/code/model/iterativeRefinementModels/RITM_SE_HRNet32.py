...
class RITM(nn.Module):
    def __init__(self, config):
        ...
        self.keypoint_num = config.Dataset.num_keypoint
    ...
    def forward(self, batch, keypoint_num=None):
        ...
        if not batch.detecting:
            ...
            input_hint_heatmap = torch.zeros([1, keypoint_num, input_image.shape[2], input_image.shape[3]]).to(self.device)
            if batch.detecting and (batch.hint.index is not None):
                for i in range(len(batch.hint.index)):
                    heatmap = self.heatmap_maker.coord2heatmap(torch.Tensor([[batch.hint.coords[i]]]))
                    input_hint_heatmap[0, batch.hint.index[i]] = heatmap[0][0]
        ...
        if batch.is_training:
        ...
        elif batch.detecting:  # detect
            # forward
            if self.config.Model.no_iterative_training:
                pred_heatmap = None
            else:
                if batch.hint.index is None:
                    pred_heatmap = torch.zeros_like(input_hint_heatmap)
                else:
            ...
            out = Munch.fromDict({'pred':{'sargmax_coord':pred_coord, 'heatmap':pred_heatmap}, })
            return out, batch
        else: # test
            # forward
            if self.config.Model.no_iterative_training:
    ...
    def find_worst_pred_index(self, batch, pred_heatmap):
        ...
        with torch.no_grad():
            ...
            for j, idx in enumerate(batch.hint.index):
                if idx is not None:
                    batch_metric_value[j, idx] = torch.full_like(batch_metric_value[j,idx], -1000)  # 把該keypoint的MRE弄到最小
